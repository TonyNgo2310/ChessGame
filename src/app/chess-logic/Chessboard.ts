...
import { CheckState, Color, Coords, FENChar, GameHistory, LastMove, MoveList, MoveType, SafeSquares } from "./model";
import { Piece } from "./pieces/piece"
import { error } from "node:console";
import { FENConverter } from "./FENConverted";
...
export class ChessBoard {
    ...
    private canCastle(king: King, kingSideCastle: boolean): boolean{
        ...
        if(this.chessBoard[KingPositionX][firstNextKingPositionY]|| this.chessBoard[KingPositionX]
            [secondNextKingPositionY]) return false;
    ...
    public move(prevX: number, prevY: number, newX: number, newY: number, promotedPieceType: FENChar|null): void{
        if(this._isGameOver) throw new Error("Game is over, you cant move");
        ...
        this.handlingSpecialMove(piece,prevX, prevY, newX, newY, moveType);
        // update the board
        ...
        this.updateThreeFoldRepetiontionDictionary(this._boardAsFEN);
        this._isGameOver = this.isGameFinished();
    }
    
    private handlingSpecialMove(piece: Piece, prevX: number, prevY: number, newX: number, newY: number, moveType: Set<MoveType>): void{
    ...
    private isGameFinished(): boolean{
        ...
        if(!this._safeSquares.size){
            if(this._checkState.isInCheck){
                ...
                this._gameOverMessage = prevPlayer + "Won";
            }
        ...
        if(this.threeFoldRepetitionFlag){
            this._gameOverMessage="Draw due to three fold repetition";
            return true;
        }
    ...
    private updateThreeFoldRepetiontionDictionary(FEN: string): void{
